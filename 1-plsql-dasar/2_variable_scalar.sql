...
-- NOTE : 
-- a. Binary Float cocok untuk menangani data dari sensor, game engine atau perangkat IoT karena menggunakan format floating-point standar IEEE
...
-- Hanya ada dua tipe yaiut DAY TO SECOND dan YEAR TO MONTH
DECLARE
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('Interval 3: ' || v_interval_3);
END;
/
...
-- Contoh penggunaan INTERVAL DAY TO SECOND untuk menghitung 90 menit dari waktu mulai ujian
DECLARE
	v_durasi_ujian    	INTERVAL DAY TO SECOND;
	v_waktu_mulai     	TIMESTAMP := SYSTIMESTAMP;
	v_waktu_selesai		TIMESTAMP;
BEGIN
	v_durasi_ujian  := INTERVAL '0 01:30:00' DAY TO SECOND;
	v_waktu_selesai	:= v_waktu_mulai + v_durasi_ujian;

	DBMS_OUTPUT.PUT_LINE('Waktu Selesai: ' || TO_CHAR(v_waktu_selesai, 'HH24:MI:SS'));
END;
/